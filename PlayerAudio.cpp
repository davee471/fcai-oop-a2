...
{
    std::cout << "Attempting to load file: " << file.getFullPathName() << std::endl;
    std::cout << "File exists: " << (file.existsAsFile() ? "YES" : "NO") << std::endl;
    
    // check if the file is valid
    if (file.existsAsFile())
    {
        currentFile = file;
        std::cout << "Current file set to: " << currentFile.getFullPathName() << std::endl;

        // ask the juce format manager to create a reader for this file
        if (auto* reader = formatManager.createReaderFor(file))
        {
            std::cout << "File reader created successfully" << std::endl;
            std::cout << "Sample rate: " << reader->sampleRate << ", Channels: " << reader->numChannels << std::endl;
            ...
            if (!f.isNull() && f.tag())
            {
                ...
                // get metadata, check if empty then convert to string
            ...
            std::cout << "File loaded successfully! Title: " << title << std::endl;
            return true;
        }
        else
        {
            std::cout << "FAILED to create reader for file - format may not be supported" << std::endl;
            return false;
        }
    }
    else
    {
        std::cout << "File does not exist or is not a file" << std::endl;
        return false;
    }
}
